...
std::string sha256str(const std::string &msg) {
  ...
  create_digest(reinterpret_cast<const unsigned char *>(msg.data()), msg.size(),
                &digest, &digest_len);
...
uint64_t decode_base64(const std::string &encoded_string) {
  ...
  while (pos + 3 < len) {
    ...
    decoded.push_back((char0_pos << 2) |
                      (char1_pos >> 4));  // Decode the first byte
  ...
  memcpy(&decoded_int, decoded.data(),
         std::min(sizeof(decoded_int), decoded.size()));
  return decoded_int;
}
