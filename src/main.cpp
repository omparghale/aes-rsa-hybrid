 ...
 *
 * Key Features:
 * - AES and RSA implementation written from scratch (without third-party cryptography libraries).
...
int main(int argc,char* argv[])
{
  ...
  ciphertext_len = enc_aes_128_cbc(message,aeskey,iv,aes_ciphertext);
  ...
  decryptedtext_len = dec_aes_128_cbc(rsa_decryptedtext, iv,
                                      aes_ciphertext,
                                      aes_decryptedtext,
                                      aes_ciphertext.size());
  ...
  writeDecrytedMsg("test/decrypted.txt", decryptedtext_str,file_to_encrypt);
