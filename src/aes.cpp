...
#ifdef _WIN32
#include <windows.h>
#include <wincrypt.h> // for key & iv generation
 ...
 * - inv_sBox: AES inverse substitution box for decryption (used in InvSubBytes step)
...
// S-Box construction: Take the multiplicative inverse in GF(2^8) of A_i followed by affine transformation to get B_i.
const byte sBox[256] = {
...
// Inverse S-Box construction: Apply the inverse affine transformation of B_i followed by multiplicative inverse in GF(2^8) to get A_i.
...
std::vector<byte> generate16bytes()
{
  ...
  if (!CryptGenRandom(hCryptProv, 16, randNum.data()))
...
void keySchedule(const std::vector<byte> &key_0, std::vector<byte> &roundKey)
{
  ...
  {
    keys.push_back((uint32_t)(key_0[i] << 24) |
                   (uint32_t)(key_0[i + 1] << 16) |
                   (uint32_t)(key_0[i + 2] << 8) |
                   (uint32_t)(key_0[i + 3]));
  }
  keys.resize(44);
  ...
  for (uint8_t i = 1; i <= 10; ++i)
  {
    ...
    {
      keys[4 * i + j] = keys[4 * i + j - 1] ^ keys[4 * (i - 1) + j]; // other words
    }
  }
...
uint32_t g_func(uint32_t word, int round)
{
  ...
  word = (((uint32_t)(sBox[(word >> 24) & 0xff]) ^ (uint32_t)rcon[round - 1]) << 24) |
...
{
  size_t pad_len = input.size() % 16 == 0 ? 16 : 16 - (input.size() % 16);
  input.insert(input.end(), pad_len, static_cast<byte>(pad_len));
}
...
void byteSub(byte state[], size_t state_size)
{
  ...
  {
    // state[i] = sBox[((state[i] & 0xf0) >> 4) * 16 + (state[i] & 0x0f)];  // Textbook method
...
void mixColumns(byte state[], size_t state_size)
{
  ...
  {
    c[0] = gf_mul(state[i], 0x02) ^ gf_mul(state[i + 1], 0x03) ^ state[i + 2] ^ state[i + 3];
    c[1] = state[i] ^ gf_mul(state[i + 1], 0x02) ^ gf_mul(state[i + 2], 0x03) ^ state[i + 3];
    c[2] = state[i] ^ state[i + 1] ^ gf_mul(state[i + 2], 0x02) ^ gf_mul(state[i + 3], 0x03);
    c[3] = gf_mul(state[i], 0x03) ^ state[i + 1] ^ state[i + 2] ^ gf_mul(state[i + 3], 0x02);
 ...
 * 1. PKCS7 padding
 * 2. CBC mode XOR
 * 3. 10 rounds of: SubBytes->ShiftRows->MixColumns(except last round)->AddRoundKey
 */
                    ...
                    std::vector<byte> &ciphertext)
{
  ...
  const size_t pt_size = og_str.size() + (16 - (og_str.size() % 16));
  const size_t num_blocks = pt_size / 16;
  std::vector<byte> plaintext;
  plaintext.reserve(pt_size);
  plaintext.assign(og_str.begin(), og_str.end());
  pkcs7_pad(plaintext); // pad the input
  ...
  ciphertext.resize(pt_size); // preallocation for ciphertext

  const byte *pt_ptr = plaintext.data();
  ...
  {
    memcpy(curr_state, pt_ptr + (block * 16), 16); // load plaintext block as current state
...
void invMixColumns(byte state[], size_t state_size)
{
  ...
  for (uint8_t i = 0; i < state_size; i += 4)
  {
    b[0] = gf_mul(state[i], 0x0e) ^ gf_mul(state[i + 1], 0x0b) ^ gf_mul(state[i + 2], 0x0d) ^ gf_mul(state[i + 3], 0x09);
    b[1] = gf_mul(state[i], 0x09) ^ gf_mul(state[i + 1], 0x0e) ^ gf_mul(state[i + 2], 0x0b) ^ gf_mul(state[i + 3], 0x0d);
    b[2] = gf_mul(state[i], 0x0d) ^ gf_mul(state[i + 1], 0x09) ^ gf_mul(state[i + 2], 0x0e) ^ gf_mul(state[i + 3], 0x0b);
    b[3] = gf_mul(state[i], 0x0b) ^ gf_mul(state[i + 1], 0x0d) ^ gf_mul(state[i + 2], 0x09) ^ gf_mul(state[i + 3], 0x0e);
    state[i] = b[0];
    state[i + 1] = b[1];
    state[i + 2] = b[2];
    state[i + 3] = b[3];
                    ...
                    bool test_mode)
{
  ...
  const byte *rk_ptr = roundKeys.data() + roundKeys.size(); // start decryption from key_10
